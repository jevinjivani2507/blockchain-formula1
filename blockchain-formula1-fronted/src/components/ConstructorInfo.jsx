...
import { ADD_CONSTRUCTOR, REMOVE_CONSTRUCTOR } from "../Redux/ActionTypes";
...
const ConstructorInfo = (props) => {
  ...
  const addPlayer = (e) => {
    ...
    if (!isAdded) {
      // console.log(props);
      if (selectdConstructor.length + 1 <= 1) {
        if (totalPoints + props.price < 1000) {
          dispatch({ type: ADD_CONSTRUCTOR, payload: props });
          setIsAdded(!isAdded);
        } else {
          maximumLimit();
        }
      ...
      dispatch({ type: REMOVE_CONSTRUCTOR, payload: props });
      setIsAdded(!isAdded);
    }
  };

  useEffect(() => {
    // console.log(selectdConstructor);
